...
def is_patch_tumor(t):
    # If there is atleast 1 pixels with 1.
    # it should be just tumor
    return int(np.sum(t) >= 1)
...
for i in tqdm(range(int(validation_steps))):
    ...
    y_pred1_summary_prob = y_pred1_summary.mean(axis=1)
    ...
    val_prob += list(y_pred1_summary_prob)
    confusion_mtx += confusion_matrix(y_true, y_pred, labels=[0, 1])
joblib.dump(confusion_matrix, 'sgd_val_confusion_matrix.joblib.pickle')
joblib.dump(val_true, 'sgd_val_true.joblib.pickle')
joblib.dump(val_pred, 'sgd_val_pred.joblib.pickle')
joblib.dump(val_prob, 'sgd_val_prob.joblib.pickle')
...
for i in tqdm(range(int(train_steps))):
    ...
    confusion_mtx += confusion_matrix(y_true, y_pred, labels=[0, 1])
joblib.dump(confusion_matrix, 'sgd_train_confusion_matrix.joblib.pickle')
joblib.dump(train_true, 'sgd_train_true.joblib.pickle')
joblib.dump(train_pred, 'sgd_train_pred.joblib.pickle')
