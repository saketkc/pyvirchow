...
def _parse_csv(rows_string_tensor):
    ...
    columns = tf.decode_csv(
        rows_string_tensor, record_defaults=[[0.0]] * NUM_COLUMNS, field_delim="\t"
    )
    label = columns[0]
...
def random_forest(num_classes=2, num_features=46, num_trees=100, max_nodes=10000):
...
def train_rf(sess, train_df_file, batch_size=1024, num_steps=100):
    training_init_op, training_next_batch = input_fn([train_df_file], batch_size)
    previous_loss = None
    loss = None
    for epoch in range(num_steps):
        ...
        while True:
            ...
            _, loss = sess.run(
                [train_op, loss_op],
                feed_dict={X: training_features_batch, Y: training_label_batch},
            )
        acc = sess.run(
            accuracy_op, feed_dict={X: training_features_batch, Y: training_label_batch}
        )
...
def accuracy_rf(sess, df_file, batch_size=1):
    ...
    while True:
        try:
            ...
            break
        acc = sess.run(accuracy_op, feed_dict={X: features_batch, Y: label_batch})
