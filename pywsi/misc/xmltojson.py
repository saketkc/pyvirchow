...
def xmltojson(xml_path, savedir):
    ...
    for annotation in soup.findAll("annotation"):
        ...
        if annotation["partofgroup"] == "_1":
            ...
            json_dict["tumor"].append(
                {"name": annotation["name"], "vertices": tumor_coordinates}
            )
            ...
            json_dict["normal"].append(
                {"name": annotation["name"], "vertices": normal_coordinates}
            )
    ...
    with open(
        os.path.join(savedir, os.path.basename(xml_path).replace(".xml", ".json")), "w"
    ) as fw:
