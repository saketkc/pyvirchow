...
class SegDirectoryIterator(Iterator):
    ...
    def __init__(
        ...
        if color_mode not in {"rgb", "grayscale"}:
            raise ValueError(
                "Invalid color mode:", color_mode, '; expected "rgb" or "grayscale".'
            )
        ...
        if class_mode not in {"sparse", None}:
            raise ValueError(
                "Invalid class_mode:",
                class_mode,
                "; expected one of " '"sparse", or None.',
            )
            ...
            if self.target_size:
                if self.crop_mode != "none":
                    ...
                    if self.label_file_format is not "npy":
                        y = img_to_array(label, data_format=self.data_format).astype(
                            int
                        )
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                fname = "{prefix}_{index}_{hash}".format(
                    prefix=self.save_prefix, index=i, hash=np.random.randint(1e4)
                )
                img.save(
                    os.path.join(
                        self.save_to_dir,
                        "img_" + fname + ".{format}".format(format=self.save_format),
                    )
                )
...
class SegDataGenerator(object):
    def __init__(
        ...
        if crop_mode not in {"none", "random", "center"}:
            raise Exception(
                'crop_mode should be "none" or "random" or "center" ' "Received arg: ",
                crop_mode,
            )
        self.data_format = data_format
        if data_format == "channels_first":
    ...
    def random_transform(self, x, y):
        ...
        assert (
            x.shape[img_row_index] == y.shape[img_row_index]
            and x.shape[img_col_index] == y.shape[img_col_index]
        ), (
            "DATA ERROR: Different shape of data and label!\ndata shape: %s, label shape: %s"
