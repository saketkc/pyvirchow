...
def pair_random_crop(x,
    ...
    w_start, w_end = offsetw, offsetw + random_crop_size[1]
    if data_format == 'channels_first':
        return x[:, h_start:h_end, w_start:w_end], y[:, h_start:h_end, h_start:
                                                     h_end]
    elif data_format == 'channels_last':
        return x[h_start:h_end, w_start:w_end, :], y[h_start:h_end, w_start:
                                                     w_end, :]
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.target_size:
            ...
            if self.loss_shape is None and self.label_file_format is 'img':
                batch_y = np.zeros(
                    (current_batch_size, ) + self.label_shape, dtype=int)
            elif self.loss_shape is None:
                batch_y = np.zeros((current_batch_size, ) + self.label_shape)
            else:
                batch_y = np.zeros(
                    (current_batch_size, ) + self.loss_shape, dtype=np.uint8)
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    if self.data_format == 'channels_first':
                        x = np.lib.pad(
                            x, ((0, 0), (pad_h // 2, pad_h - pad_h // 2),
                                (pad_w // 2, pad_w - pad_w // 2)),
                            'constant',
                            constant_values=0.)
                        y = np.lib.pad(
                            y, ((0, 0), (pad_h // 2, pad_h - pad_h // 2),
                                (pad_w // 2, pad_w - pad_w // 2)),
                            'constant',
                            constant_values=self.label_cval)
                    elif self.data_format == 'channels_last':
                        x = np.lib.pad(
                            x, ((pad_h // 2, pad_h - pad_h // 2),
                                (pad_w // 2, pad_w - pad_w // 2), (0, 0)),
                            'constant',
                            constant_values=0.)
                        y = np.lib.pad(
                            y, ((pad_h // 2, pad_h - pad_h // 2),
                                (pad_w // 2, pad_w - pad_w // 2), (0, 0)),
                            'constant',
                            constant_values=self.label_cval)
                else:
...
class SegDataGenerator(object):
    def __init__(self,
        ...
        if crop_mode not in {'none', 'random', 'center'}:
            raise Exception(
                'crop_mode should be "none" or "random" or "center" '
    ...
    def random_transform(self, x, y):
        ...
        assert x.shape[img_row_index] == y.shape[img_row_index] and x.shape[img_col_index] == y.shape[img_col_index], 'DATA ERROR: Different shape of data and label!\ndata shape: %s, label shape: %s' % (
