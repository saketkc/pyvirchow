...
def list_pictures(directory, ext='jpg|jpeg|bmp|png|ppm'):
    return [
        ...
        for f in files if re.match(r'([\w]+\.(?:' + ext + '))', f.lower())
    ]
    ...
    def __getitem__(self, idx):
        if idx >= len(self):
            raise ValueError('Asked to retrieve element {idx}, '
                             'but the Sequence '
                             'has length {length}'.format(
                                 idx=idx, length=len(self)))
    ...
    def _flow_index(self):
        ...
        while 1:
            ...
            if self.n > current_index + self.batch_size:
                ...
                self.batch_index = 0
            self.total_batches_seen += 1
            yield self.index_array[current_index:current_index +
                                   self.batch_size]
...
class NumpyArrayIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        output = (batch_x
                  if batch_x_miscs == [] else [batch_x] + batch_x_miscs, )
        if self.y is None:
            return output[0]
...
class DirectoryIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.class_mode == 'input':
            ...
            batch_y = np.zeros(
                (len(batch_x), self.num_classes), dtype=self.dtype)
