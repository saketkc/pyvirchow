...
def random_rotation(
    ...
    x = apply_affine_transform(
        x, theta=theta, channel_axis=channel_axis, fill_mode=fill_mode, cval=cval
    )
    return x
...
def random_shift(
    ...
    x = apply_affine_transform(
        x, tx=tx, ty=ty, channel_axis=channel_axis, fill_mode=fill_mode, cval=cval
    )
    return x
...
def random_shear(
    ...
    x = apply_affine_transform(
        x, shear=shear, channel_axis=channel_axis, fill_mode=fill_mode, cval=cval
    )
    return x
...
def random_zoom(
    ...
    x = apply_affine_transform(
        x, zx=zx, zy=zy, channel_axis=channel_axis, fill_mode=fill_mode, cval=cval
    )
    return x
        ...
        if img.size != width_height_tuple:
            if interpolation not in _PIL_INTERPOLATION_METHODS:
                raise ValueError(
                    ...
                    "methods are {}".format(
                        interpolation,
                        ", ".join(list(_PIL_INTERPOLATION_METHODS.keys())),
                    )
                )
    ...
    def apply_transform(self, x, transform_parameters):
        ...
        if transform_parameters.get("channel_shift_intensity") is not None:
            x = apply_channel_shift(
                x, transform_parameters["channel_shift_intensity"], img_channel_axis
            )
    ...
    def fit(self, x, augment=False, rounds=1, seed=None):
       ...
       """
        ...
        if self.zca_whitening:
            if scipy is None:
                raise ImportError(
                    "Using zca_whitening requires SciPy. " "Install SciPy."
                )
