...
def get_model():
    ...
    model.add(
        Convolution2D(100, (5, 5), strides=(2, 2), activation="elu", padding="same")
    )
    model.add(MaxPooling2D())
    model.add(
        Convolution2D(200, (5, 5), strides=(2, 2), activation="elu", padding="same")
    )
    model.add(MaxPooling2D())
    ...
    for idx, offset in enumerate(list(range(0, n_samples, batch_size))):
        all_batch_samples.append(
            (
                idx,
                model,
                all_samples.iloc[offset : offset + batch_size],
                batch_size,
                patch_size,
                img_mask_dir,
            )
