...
class MacenkoNormalization(object):
    ...
    def get_concentrations(self, image, stain_matrix):
        ...
        if self.maskout_white:
            nonwhite_mask = get_nonwhite_mask(image, self.nonwhite_threshold).reshape(
                (-1,)
            )
            OD = OD[nonwhite_mask]
        coefs = (
            spams.lasso(OD.T, D=stain_matrix.T, mode=2, lambda1=self.lambda1, pos=True)
            .toarray()
            .T
        )
        return coefs
    ...
    def transform(self, source_images):
        ...
        for source_image in source_images:
            ...
            reconstructed = np.dot(
                source_concentrations, self.target_stain_matrix
            ).reshape(source_image.shape)

            reconstructed = (
                OD2RGB(reconstructed).reshape(source_image.shape).astype(np.uint8)
            )
    ...
    def get_hematoxylin_channel(self, source_image=None, total=True):
        ...
        if source_image is not None:
            ...
            concentrations = self.get_concentrations(source_image, stain_matrix_source)
        else:
        ...
        if total:
            H = np.multiply(
                ...
                np.reshape(self.target_stain_matrix[0, :], (1, 3)),
            ).reshape(h, w, 3)
        else:
    ...
    def get_eosin_channel(self, source_image=None, total=True):
        ...
        if total:
            E = np.multiply(
                ...
                np.reshape(self.target_stain_matrix[1, :], (1, 3)),
            ).reshape(h, w, 3)
        else:
